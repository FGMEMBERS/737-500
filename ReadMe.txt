*********************

README

*********************

This is the 737-500 in Progress and under GNU GPL v2.0

The work is based on the work of David Culp (FDM) and Innis Cunningham (3d-model)


The Boeing 737-500 is now under werk to be improved and come much closer to realitity.

Authors:
Current main author: Heiko Schulz (Heiko.H.Schulz@gmx.net)
3d-model: Heiko Schulz, Innis Cunningham
3d-cockpit: Heiko Schulz, Ispana FlightGear Team, Emmanuel Barranger
3d-model resize to -500 series : Isaias V. Prestes
For seeing the 3d-cockpit in progress pan the view so the 2d-panel will dissapear

helping Info

Changelog:
12/15/2010:
___________

-adjusted the 3d model to -500 series size and characteristics.
-paintkit for -500 series.

02/18/2010:
___________

-fixed animation errors on elevator
-added trim animation to the stab
-fixed material animation bug on nosegear doors
-added KLM Livery by Nick (?) and Lufthansa Retrolivery 1986 by myself
-added metal texture to engine exhaust
-refinded Paintkit
-fixed "hover" of the model
-fdm fixed as it had a nose down attitude at high speed and low weight
-added a more proper weight locations from a free available weight and balance Chart of a 737-500:http://www.aaleda.co.nz/trim-373.pdf
 but kg->lbs was calculated wrong which gives overweight- Will be changed later.
-autopilot stable now in all altitudes, every weight and on glideslope until approx. 25-0ft
-fixed spoiler and speedbrake animation which was accidently changed




